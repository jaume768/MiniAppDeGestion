...
import { ticketsAPI, clientesAPI, articulosAPI } from '../../../services/api';
...
export default function TicketsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [tkData, clientesData, articulosData] = await Promise.all([
          ticketsAPI.getAll(),
          clientesAPI.getAll(),
          articulosAPI.getAll()
        ]);
        tkData.sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
        setTickets(tkData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const nuevo = await ticketsAPI.create(payload);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="page-title">Gestión de Tickets</h2>
        <button className={`btn ${showForm ? 'btn-outline-secondary' : 'btn-primary'}`} onClick={() => setShowForm(prev => !prev)}>
          {showForm ? <><IconTimes className="me-1" />Cancelar</> : <><IconPlus className="me-1" />Nuevo Ticket</>}
        </button>
      </div>
      {error && <div className="alert alert-danger">{error}</div>}
      {showForm && (
        <div className="card mb-4">
          <div className="card-header"><h5>Crear Ticket</h5></div>
              ...
              <div className={styles.itemsSection}>
                ...
                {itemsForm.length > 0 && (
                  <table className="table mb-3">
                    ...
                    <tbody>
                      {itemsForm.map((it,i) => (
                        <tr key={i}><td>{it.articulo_nombre}</td><td>{it.cantidad}</td><td>{it.precio_unitario}€</td><td>{it.subtotal.toFixed(2)}€</td><td><button type="button" className="btn btn-sm btn-danger" onClick={() => removerItem(i)}><IconTrash /></button></td></tr>
                      ))}
                    </tbody>
                    <tfoot><tr><td colSpan="3" className="text-end">Total:</td><td>{calcularTotal()}€</td><td></td></tr></tfoot>
                  </table>
                )}
              </div>
      ...
      <TableComponent
        ...
        headers={[
          { name: 'ID', key: 'id' },
          { name: 'Cliente', key: 'cliente_nombre', render: t => t.cliente_nombre || `Cliente ID: ${t.cliente}` },
          { name: 'Fecha', key: 'fecha', render: t => new Date(t.fecha).toLocaleDateString() },
          { name: 'Total', key: 'total', render: t => `${t.total}€` }
        ]}
        data={tickets}
        onRowClick={t => window.location.href = `/pages/ventas/tickets/${t.id}`}
        actionButtons={t => (
          <div className="d-flex gap-1">
            <Link href={`/pages/ventas/tickets/${t.id}`} className="btn btn-sm btn-outline-primary" onClick={e => e.stopPropagation()}><IconEye /></Link>
            <button className="btn btn-sm btn-outline-danger" onClick={e => { e.stopPropagation(); eliminarTicket(t.id); }}><IconTrash /></button>
          </div>
        )}
      />
    </div>
