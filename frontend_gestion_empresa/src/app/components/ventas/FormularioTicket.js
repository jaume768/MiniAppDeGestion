...
import { ticketsAPI, clientesAPI, articulosAPI } from '@/app/services/api';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (ticket) {
          const ticketCompleto = await ticketsAPI.getById(ticket.id);
          setFormData({
            cliente: ticketCompleto.cliente,
            fecha: new Date(ticketCompleto.fecha).toISOString().split('T')[0],
            observaciones: ticketCompleto.observaciones || ''
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (ticket) {
        ...
        const itemsActuales = await ticketsAPI.getItems(ticket.id);
        for (const item of itemsActuales) {
          await ticketsAPI.removeItem(ticket.id, item.id);
        }
        
        // 3. Agregar nuevos items
        for (const item of itemsFormateados) {
          await ticketsAPI.addItem(ticket.id, item);
        }
      } else {
        ...
        await ticketsAPI.create(nuevoTicketData);
      }
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.formActions}>
          ...
          >
            ...
            {saving ? 'Guardando...' : (ticket ? 'Actualizar' : 'Crear')}
          </button>
        </div>
      </form>
    </div>
